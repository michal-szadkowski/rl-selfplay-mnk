...
class OpponentPool:
    ...
    def _evict_worst_performers(self) -> None:
        ...
        sorted_indices = sorted(range(len(self.opponents)), key=lambda i: self.opponents[i]['mean_reward'])
    ...
    def _calculate_weight(self, opponent: dict) -> float:
        ...
        # Performance weight: opponents with higher mean reward against agent get higher weight
        performance_weight = max(0.1, opponent['mean_reward'] + 1.0)  # Ensure positive weight
